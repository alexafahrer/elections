...
presidents_winner <- reactive({presidents |>
...
swing_states <- reactive({presidents |>
  filter(party_simplified %in% c("DEMOCRAT", "REPUBLICAN"),
         year == input$input_year) |>
...
presidents_datatable <- presidents |>
  ...
  rowwise() |>
  mutate(percentage_of_votes = scales::percent(candidatevotes/totalvotes, accuracy = 0.01), 
  ...
  rename("Year" = "year", "State" = "state", "Candidate Name" = "candidate", 
