...
| [Dashboard](#dashboard)   | Interactive dashboard to select a presidential election year from 1970 to 2020 and explore trends through visualizations.  |
...
: Tabs on Dashboard {.striped .hover tbl-colwidths="[25,75]"}
:::

::: {.card title="About the Data" .flow}
...
swing_states_binary <- reactive({
  presidents |>
    filter(
      party_simplified %in% c("DEMOCRAT", "REPUBLICAN"),
      year == input$input_year
    ) |>
    mutate(swing_state = if_else(state_po %in% 
                                   c("NM", "NV", "IA", "FL", "VA", "OK", "TN"), TRUE, FALSE)) |>
...
swing_states_red_blue <- reactive({
  presidents |>
    filter(
      party_simplified %in% c("DEMOCRAT", "REPUBLICAN"),
      year == input$input_year, 
      state_po %in% c("NM", "NV", "IA", "FL", "VA", "OK", "TN")
    ) |>
    select(state_fips, party_simplified) |>
    rename(fips = state_fips, values = party_simplified)
})
...
```{r}
#| title: "State of Interest #1: North Carolina"
...
output$swing_states_map <- renderPlot({
  plot_usmap("states", data = swing_states_binary()) +
    scale_fill_manual(values = c("#D3D3D3", "#BA68C8")) +
...
-   Team member 1: My name is [Ryan Silver](https://github.com/ryan-silver) and I am a sophomore from St. Louis majoring in Economics and minoring in Statistical Science. I love playing pickle ball and racquetball in my free time.
-   Team member 2: My name is [Alexa Fahrer](https://github.com/alexafahrer) and I am a sophomore from Phoenix double majoring in Public Policy and Statistics with a minor in Education. In my free time, I enjoy watching movies, cheering on the 49ers, and playing pickle ball.
