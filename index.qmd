...
The winner of the popular vote has gone on to win the presidency in every election shown except for in 2000 and 2016.
:::
...
presidents_winner <- reactive({
  ...
  summarize(perc = sum(candidatevotes) / sum(totalvotes))
})
...
# Elections {#elections scrolling="false"}
...
output$winner_name <- renderText({
  candidate_name <- presidents_winner() |>
    ...
    pull(candidate)
  #Case for Joe Biden 
  if (candidate_name == "BIDEN, JOSEPH R. JR") {
    formatted_name <- "Joseph R. Biden Jr"
  } 
  #Case for HW
  else if (candidate_name == "BUSH, GEORGE H.W.") {
    formatted_name <- "George H.W. Bush" 
  }
  else {
    parts <- str_split(candidate_name, ", ")[[1]]
    first_part <- trimws(parts[2])  
    last_part <- trimws(parts[1])  
    formatted_name <- paste(first_part, last_part, sep=" ")
    formatted_name <- tolower(formatted_name)
    formatted_name <- toTitleCase(formatted_name)
  }
  formatted_name
})
...
winner_name_reactive <- reactive({
  presidents_winner() |>
    filter(perc == max(perc)) |>
    pull(candidate)
})

output$winner_party <- renderText({
  presidents |>
    filter(
      candidate == winner_name_reactive()
      ) |>
    slice(1) |>
    pull(party_simplified) |>
    tolower() |>
    toTitleCase()
})
```
...
output$loser_name <- renderText({
  ...
  #Case for McCain
  else if (candidate_name == "MCCAIN, JOHN") {
    formatted_name <- "John McCain" 
  }
  else {
    parts <- str_split(candidate_name, ", ")[[1]]
    first_part <- trimws(parts[2])  
...
loser_name_reactive <- reactive({
  presidents_winner() |>
    filter(perc == min(perc)) |>
    pull(candidate)
})

output$loser_party <- renderText({
  presidents |>
    filter(
      candidate == loser_name_reactive()
      ) |>
    slice(1) |>
    pull(party_simplified) |>
    tolower() |>
...
# Write Up {#write-up scrolling="true"}

::: {.card .flow title="Introduction"}
...
::: {.card .flow title="Project Goals & Overview"}
...
::: {.card .flow title="Why a Quarto Dashboard"}
